      ...
      {content?.map((item) => {
        ...
        return (
          <div className={`flex flex-col items-center ${lightTheme ? 'bg-center bg-cover': 'bg-squid-ink'}`} style={{ backgroundImage: lightTheme ? `url(${lightBackgroundImage})` : null }}>
            <div className="flex flex-col items-center px-6 py-18 md:py-32 lg:py-40 xl:py-48 max-w-8xl">
              ...
              {item.photos && (
                <div className="flex flex-col md:flex-row">
                  {item.photos.map((photo, idx) => (
                    <div className={`w-full md:w-1/3 my-6 md:my-0 ${idx + 1 !== item.photos.length ? 'md:mr-6' : ''}`}>
